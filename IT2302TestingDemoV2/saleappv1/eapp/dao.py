...
def add_user(name, username, password, avatar):
    ...
    if len(username) < 5:
        raise ValueError ("Username phai co it nhat 5 ky tu")
    if len(password_raw)<8:
        raise ValueError ("Password phai co it nhat 8 ky tu")
    if not (any(c.isalpha() for c in password_raw) and any(c.isdigit()for c in password_raw)):
        raise ValueError ("Password phai chua ca chu va so")
